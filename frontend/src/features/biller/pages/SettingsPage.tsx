...
// ========== ENHANCED ACCOUNT INFO CARD – dark mode ==========
function AccountInfoCard({ user, billerDetails }: { user: any; billerDetails: any }) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="flex items-center gap-3 p-3 rounded-xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 hover:border-red-100 dark:hover:border-red-800 transition-all duration-200 group">
          ...
          <div className="flex-1">
            ...
            <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400">
              {user.role === "BILLER_USER" ? t("biller_user") : user.role || t("biller_user")}
            </span>
          </div>
        </div>
        ...
        {billerDetails?.name && (
          <div className="flex items-center gap-3 p-3 rounded-xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 hover:border-red-100 dark:hover:border-red-800 transition-all duration-200 group">
            ...
            <div className="flex-1">
              <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">{t("biller_name")}</p>
              <p className="text-sm font-medium text-gray-800 dark:text-gray-200">{billerDetails.name}</p>
            </div>
          </div>
        )}
        ...
        {billerDetails?.id && (
          <div className="flex items-center gap-3 p-3 rounded-xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 hover:border-red-100 dark:hover:border-red-800 transition-all duration-200 group">
            ...
            <div className="flex-1">
              <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">{t("biller_id")}</p>
              <p className="text-sm font-mono text-gray-600 dark:text-gray-400">{billerDetails.id.slice(0, 12)}...</p>
            </div>
          </div>
        )}
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-white dark:bg-gray-800 rounded-lg">
              {settings.theme === "dark" ? (
                <svg className="w-5 h-5 text-indigo-600 dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>
              ) : (
      ...
      <div className="bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-950/30 dark:to-pink-950/30 rounded-xl p-5 border border-purple-100 dark:border-purple-800">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-white dark:bg-gray-800 rounded-lg">
              <svg className="w-5 h-5 text-purple-600 dark:text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129" /></svg>
            </div>
            <div>
          ...
          <select value={settings.language} onChange={(e) => updateSetting("language", e.target.value)} className="px-4 py-2 border rounded-lg bg-white dark:bg-gray-800 focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none" disabled={loading}>
...
export default function BillerSettingsPage() {
  ...
  const [billerDetails, setBillerDetails] = useState<any>(null);
  ...
  const fetchUserDetails = useCallback(async () => {
    try {
      ...
      if (response.data.status === "SUCCESS") {
        ...
        localStorage.setItem("biller_user_cache", JSON.stringify(actualUser));
        localStorage.setItem("biller_user_cache_time", String(Date.now()));

        const billerId = actualUser.billerId || actualUser.biller_id;
        if (billerId) {
          try {
            const billerRes = await api.get(`/billers/${billerId}`);
            if (billerRes.data.status === "SUCCESS") {
              setBillerDetails(billerRes.data.data);
              localStorage.setItem("biller_details_cache", JSON.stringify(billerRes.data.data));
            }
          } catch (err) {
            console.error("Failed to fetch biller details", err);
          }
        }
      } else {
      ...
      const cachedUser = localStorage.getItem("biller_user_cache");
      const cachedBiller = localStorage.getItem("biller_details_cache");
      if (cachedUser) {
        setUserDetails(JSON.parse(cachedUser));
        if (cachedBiller) setBillerDetails(JSON.parse(cachedBiller));
      } else {
  ...
  if (loading && !userDetails) {
    return (
      <DashboardLayout title={t("settings")} links={billerLinks}>
  ...
  return (
    <DashboardLayout title={t("settings")} links={billerLinks}>
      <div className="space-y-6 pb-8">
        ...
        <AccountInfoCard user={userDetails} billerDetails={billerDetails} />
