    ...
    try {
      ...
      if (res.status === "SUCCESS") {
        ...
        if (mustChangePassword) {
          // Redirect to change-password page with tempToken
    ...
    <PublicLayout>
      <div className="min-h-[80vh] flex justify-center items-center py-16 px-4 relative overflow-hidden">
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="w-full max-w-md">
          <motion.div variants={fadeInScale} className="bg-white rounded-2xl shadow-2xl overflow-hidden">
            {/* Header */}
            ...
            <div className="p-8 space-y-6">
              {successMessage && (
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="bg-green-50 border-l-4 border-green-600 p-4 rounded-lg">
                  <p className="text-green-700 text-sm font-medium">{successMessage}</p>
                </motion.div>
              )}
              {apiError && (
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="bg-red-50 border-l-4 border-red-600 p-4 rounded-lg">
                  <p className="text-red-700 text-sm font-medium">{apiError}</p>
                </motion.div>
              )}
              {/* Email Field */}
              <motion.div variants={fadeInUp} className="space-y-2">
                <label className="text-sm font-semibold text-gray-700 flex items-center gap-2"><EnvelopeIcon className="w-4 h-4 text-red-600" /> Email Address</label>
                <div className="relative group">
                  <input type="email" placeholder="you@example.com" className={`w-full border-2 rounded-xl p-3 pl-11 outline-none transition-all duration-200 ${fieldErrors.email ? "border-red-500 focus:border-red-500 bg-red-50" : "border-gray-200 focus:border-red-400 focus:ring-2 focus:ring-red-200"}`} value={email} onChange={(e) => { setEmail(e.target.value); setFieldErrors((prev) => ({ ...prev, email: "" })); }} />
                ...
                {fieldErrors.email && <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-red-500 text-sm">{fieldErrors.email}</motion.p>}
              </motion.div>
              {/* Password Field */}
              <motion.div variants={fadeInUp} className="space-y-2">
                <label className="text-sm font-semibold text-gray-700 flex items-center gap-2"><LockClosedIcon className="w-4 h-4 text-red-600" /> Password</label>
                <div className="relative group">
                  <input type={showPassword ? "text" : "password"} placeholder="••••••••" className={`w-full border-2 rounded-xl p-3 pl-11 pr-12 outline-none transition-all duration-200 ${fieldErrors.password ? "border-red-500 focus:border-red-500 bg-red-50" : "border-gray-200 focus:border-red-400 focus:ring-2 focus:ring-red-200"}`} value={password} onChange={(e) => { setPassword(e.target.value); setFieldErrors((prev) => ({ ...prev, password: "" })); }} />
                  ...
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">{showPassword ? <EyeSlashIcon className="w-5 h-5" /> : <EyeIcon className="w-5 h-5" />}</button>
                </div>
                {fieldErrors.password && <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-red-500 text-sm">{fieldErrors.password}</motion.p>}
              </motion.div>
              {/* Login Button */}
              <motion.div variants={fadeInUp}>
                <button onClick={handleLogin} disabled={loading} className={`w-full rounded-xl py-3 font-bold text-white transition-all duration-200 flex items-center justify-center gap-2 ${loading ? "bg-red-300 cursor-not-allowed" : "bg-gradient-to-r from-red-600 to-red-500 hover:from-red-700 hover:to-red-600 shadow-lg hover:shadow-xl"}`}>
              ...
              <motion.div variants={fadeInUp} className="text-center">
                <a href="/forgot-password" className="text-sm text-red-600 hover:text-red-700 font-semibold">Forgot password?</a>
              </motion.div>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="text-center mt-6">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/80 backdrop-blur-sm rounded-full shadow-sm">
              ...
              <span className="text-xs text-gray-600">Secure login • Your data is protected</span>
            </div>
          </motion.div>
        </motion.div>
      </div>
